...
class koshkaCard{constructor(obj,)
    {this.name = obj.name,        
    ...
    data.push(new koshkaCard(cat))}
    console.log(data)
    let html = ""
    for(let val of data){  
        let fv = (val.favourite ? "Любимый" : 'Не любимый')
    ...
    <h3>${val.name}</h3>    
    <h3>${val.age} y.o.</h3>       
    <p>Рейтинг - ${val.rate}</p>    
    <p>${fv}</p>    
    <p>${val.description}</p>
    </div>
    <img src="${val.img_link}" alt="koshak)))))">    
    </div>
    `}    
